...
Chat 端默认进入集中式 Approval Center：同一受信任 OAuth 客户端与 `openai/session` 下的待审批操作集中展示，
已处理操作自动折叠到“已处理”区，有效的本对话持续授权单独列出并可立即撤销。单张审批卡仍可按 approvalId 重新打开。
新版宿主调用 `review_approval` 时省略 `approvalId` 即打开审批中心；若某个已连接的宿主仍缓存旧 schema 并强制要求该字段，
可传保留兼容值 `__approval_center__`。该值只选择“审批中心”视图，不代表任何真实审批编号，不批准或执行操作，也不改变权限范围。
默认按钮仍是单次批准。非 Codex Chat 审批卡固定提供“本对话始终批准（受限范围）”：
租约同时绑定已验证 OAuth 客户端和 `openai/session`，最长只存活到当前审批的固定截止时间，
不因使用续期、不写数据库、服务重启即失效。点击该按钮会明确批准当前请求一次，同时只对项目 review、
隔离 worktree、已经由用户明确批准过的完全相同 `run_process`，以及完全相同根目录与完整参数的 `exec_command` / `bash` / `host_command`
建立安全租约。Shell 的持续授权只匹配该精确命令请求；命令、参数或工作区变化后立即回到正常审批。PTY 输入、Codex 委派、凭据或服务状态、
安全/执行配置、删除/移动和未知能力同样不会继承可重复执行权限，仍逐次审批。

普通完整哈希保护的小批量补丁本来就不需要逐次批准；真正会触发审批的敏感、大批量、删除/移动补丁
不会因为对话租约而放行。参数、工作区、客户端或逻辑会话超出租约范围时自动回到正常审批。
用户可在同一对话调用 `revoke_conversation_approvals` 立即撤销全部租约；该工具只能收紧权限。
用户批准非 Codex 操作后，原 Chat 逻辑会话应先直接重试完全相同的原始操作；不要把 `claim_approval`
当作每次批准后的固定前置步骤。只有宿主在用户点击审批后或租约有效期内切换了 `openai/session`，导致原批准收据
不能由当前 session 消费时，Chat 才调用 `claim_approval(approvalId)` 把该批准的收据/租约 lineage 交接到当前
session，再重试原始精确操作或安全租约覆盖的操作。若第一次精确重试直接返回新的 `OWNER_APPROVAL_REQUIRED`
与新 approvalId，不要要求用户再次批准；应使用**原 approvalId**执行上述 claim，claim 成功后只重试原始精确操作一次，
claim 失败则停止，不能继续制造新的审批链。原始一次执行权消费后，
服务只在内存中保留用于迁移仍有效租约的 lineage，不会重新授予那次执行权；TTL 到期、主动撤销或服务重启都会清除它。
claim 只允许同一受信任 OAuth client，不批准新操作，也不会放宽参数、工作区或安全操作集合。
...
2. 已启用 Chat 审批的客户端优先调用 `review_approval` 且省略 `approvalId` 展示集中审批中心；若宿主仍缓存旧的
   必填 schema，则使用 `review_approval({ approvalId: "__approval_center__" })`。只需重新打开某一个请求时才传真实
   `approvalId`。否则用户自己打开 Owner 地址。核对工具、完整参数、路径/文件指纹和选定模型。
3. 在卡片中选择单次批准或拒绝。Owner 兼容页面仅在登录身份需要重新验证时输入密码，不要把密码交给 Chat。
   拒绝是该审批记录的终态。如果用户之后明确要求“重新申请”，Chat 可对原拒绝编号调用
   `reissue_approval`；它只准备一次精确的重新申请，不批准也不执行。随后重试完全相同的原操作
   才会产生新的 `pending` 审批，仍需用户再次决定。不同客户端/会话不能复用此入口。
...
5. Shell、补丁及其他高风险操作仍需回 Chat 重试完全相同的操作。同一 `openai/session` 先直接消费批准；
   只有 logical session 发生漂移且原收据无法消费时，才使用 `claim_approval` 迁移批准 lineage。若用户明确选择“本对话始终批准（受限范围）”，
   完全相同的 `exec_command` / `bash` / `host_command` 后续调用可在固定 TTL 内命中各自的精确命令租约；任何命令、参数或工作区变化都需要新授权。
...
启用后，Chat 优先调用 `review_approval`（省略 `approvalId`）展示本对话审批中心；缓存旧必填 schema 的宿主可传
`__approval_center__` 作为兼容入口。中心内每个待审批项仍使用独立的私有 decision token，
不会形成批量批准能力。你也可以通过 `review_approval` 单独打开一个请求。你在卡片内
选择“批准一次”“本对话始终批准（受限范围）”或“拒绝”，不需要把 Owner 密码交给 Chat。安全操作按钮会批准当前请求一次，
但后续自动放行仍受上述安全集合约束；精确 Shell 租约只覆盖完全相同的命令请求。批准 Codex 请求会
复用已有的单轮自动提交；未纳入明确租约的其他高风险操作仍只批准精确的一次重试。卡片会尝试
...
普通读取、搜索和符合已有约束的小补丁继续按原规则执行。对话租约是明确用户点击创建的临时、
窄范围能力，不是模型推断的“默认同意”，也没有新增自动审批 Codex/Shell 的风险分类器或永久授权。
...
拒绝的请求不会自动转为同意，也不会因普通重试自动生成新请求；只有同一受信任会话在用户明确
要求后调用 `reissue_approval` 才进入回游模式的 RELAUNCH，再由原操作创建全新的待审批记录。页面要求
...
进行选择。规则由 Canonical Intent IR 与奇美拉路由层实现，是可审查的确定性启发式，不是额外的模型调用：长任务，以及包含
