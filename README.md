...
启用执行策略后，每轮使用明确的批准模型，或由配置的路由在允许列表内选择，
并校验客户端版本、权限回执和对应轮次的运行证据。普通文件工具不启动模型推理。

启用 Owner 授权时，Codex 任务经用户在审批页批准后会自动提交一次，不再要求
回聊天窗口重试才能开始；页面显示任务编号，聊天端继续查询结果。旧客户端的
相同请求重试只恢复提交收据，不重复执行。此能力不会自行开启后台聊天循环或永久提权。

可以对明确受信任的 ChatGPT OAuth 客户端启用对话内审批卡片，在卡片中批准或
拒绝一次操作，无需每次输入 Owner 密码。默认不启用，原 Owner 页面保留；
卡片仍有请求绑定、有效期和服务端校验，不代表模型可以自我授权。

任务审批默认 30 分钟，可配置 30–120 分钟；页面 Cookie 与请求使用同一固定截止时间，
刷新不会续期。它与登录期限、任务运行超时和提供方空闲回收不是同一个设置。

可选 `high_risk_only` 审批配置让普通文件操作自动通过，高危能力仍需确认。
Owner 登录可以设置为固定 12 小时验证一次，刷新令牌不会延长该期限；有效期内
高危确认无需重复输入密码。任意 Shell 和尚未接通内部逐项审批的 Codex 委派
仍保留确认，不能用“普通编程”标签替代权限约束。详见授权说明中的范围与限制。
...
标准持久化工作区使用 32 项 LRU 缓存，对象与物理根锚点共同保存；驱逐不删除
持久身份，恢复仍重新检查原锚点。可选 Codex 用量来自提供方实际通知，不是按
调用次数或文本长度估算；缺失回执不当作零，也不能单独推出完整账单或节省比例。
...
图中是开发操作的主路径；健康、OAuth 发现和审批展示有各自的入口，并非每个 HTTP 请求都打开工作区。
...
Chat 的高风险操作可使用独立 Owner 页面逐次批准；Codex 可在明确允许的
GPT-6 Astra / GPT-5.6 Sol 之间按任务路由，不借此扩大权限。配置与确认步骤见
