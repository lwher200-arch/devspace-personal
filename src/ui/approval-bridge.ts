...
import { ApprovalError, APPROVAL_ERROR_CODES, REVIEW_APPROVAL_TOOL, REVIEW_APPROVALS_TOOL, DECIDE_APPROVAL_TOOL, REVOKE_CONVERSATION_APPROVALS_TOOL, RECYCLE_APPROVALS_TOOL } from '../approval-protocol.js';
...
export function createApprovalBridge(standard: () => StandardBridge | null, legacy: () => ChatGptApprovalBridge | undefined): ApprovalCardBridge {
  ...
  return {
    ...
    center: async () => {
      const decoded = decodeToolResult(await call(REVIEW_APPROVALS_TOOL, {}));
      if (decoded.kind !== 'approval-center') throw new ApprovalError('APPROVAL_RESPONSE_MISMATCH');
      return decoded.center;
    },
    revoke: async () => {
      const result = await call(REVOKE_CONVERSATION_APPROVALS_TOOL, {});
      try {
        const text = result.content.find(item => item.type === 'text');
        const payload = JSON.parse(text?.type === 'text' ? text.text : '{}');
        if (typeof payload.revoked === 'number' && Number.isInteger(payload.revoked) && payload.revoked >= 0) return payload.revoked;
      } catch { /* handled below */ }
      throw new ApprovalError('APPROVAL_RESPONSE_MISMATCH');
    },
    recycle: async () => {
      const result = await call(RECYCLE_APPROVALS_TOOL, {});
      try {
        const text = result.content.find(item => item.type === 'text');
        const payload = JSON.parse(text?.type === 'text' ? text.text : '{}');
        if (typeof payload.recycled === 'number' && Number.isInteger(payload.recycled) && payload.recycled >= 0) return payload.recycled;
      } catch { /* handled below */ }
      throw new ApprovalError('APPROVAL_RESPONSE_MISMATCH');
    },
    notify: async message => {
      ...
      if (app?.getHostCapabilities()?.message) {
        ...
        if (typeof compatibility?.sendFollowUpMessage === 'function') {
          await compatibility.sendFollowUpMessage({ prompt: message, scrollToBottom: false });
          return;
        }
        throw new ApprovalError('CHAT_BRIDGE_UNAVAILABLE');
