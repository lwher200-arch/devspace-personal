...
启动时若出现 `Database migration history is incompatible`，先核对报错中的
迁移版本、名称与实际构建。个人版保留第 7 号 `local-agent-execution-contract`
迁移；未知版本或同版本异名会在应用任何待执行迁移前拒绝，初始化失败会关闭连接。

不要删除迁移记录、重编号或清空数据库来绕过拒绝。停止自己拥有的服务后，保留
完整数据库备份，再使用与该状态兼容的构建；必要时恢复已验证的构建与状态备份。
对仍启用 WAL 的活动数据库，不要只复制主 `.sqlite` 文件，使用 SQLite backup
或在所有相关进程关闭后备份完整状态。无效数据库不会在启动时自动修复。

已有 WAL 数据库的并发升级有回归覆盖；首次将 DELETE 模式切换到 WAL 的并发
启动曾在 Windows 测试中返回 `SQLITE_BUSY`，本轮未增加重试或改变日志模式策略。
保持一个明确的服务启动方，先查清竞争进程，不要任意结束其他进程。
...
标准持久化工作区缓存采用 32 项 LRU。缓存驱逐不删除工作区 ID 或数据库记录，
也不撤销已提交任务；下次使用时根据原锚点恢复，目标变化仍会被拒绝。

内存中的技能激活状态不会随恢复自动保留。如果技能附属文件读取被拒绝，先重新
读取已经声明的 `SKILL.md`，不要扩大允许目录。需要更新 checkout 的规则和目录时，
对同一路径使用 `open_workspace` 的 `refreshContext: true`；不要为刷新创建新工作树。
